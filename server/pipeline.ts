...
async function runTranscriptionAttempt(
  ...
  const notify = (p: number, m: string) => {
    if (typeof options.onProgress === "function")
      options.onProgress({ progress: p, message: m });
  };
  ...
  try {
    ...
    if (!options.workingFilePath && isRemoteAudioPath(workingFilePath)) {
      ...
      tempFilePath = path.join(
        uploadDir,
        `temp_transcribe_${crypto.randomUUID()}${ext}`
      );
    ...
    try {
      ...
      for (const model of modelsToTry) {
        ...
        try {
          if (isLargeFile) {
            ...
            const sentToStt = Number(
              transcriptionDiagnostics.chunksSentToStt || 0
            );
            const failedAtStt = Number(
              transcriptionDiagnostics.chunksFailedAtStt || 0
            );
            if (sentToStt > 0 && failedAtStt === sentToStt) {
              const error: any = new Error(
                "Transkrypcja STT nie powiodla sie dla zadnego modelu."
              );
            ...
            transcriptionDiagnostics = {
              ...
              chunksReturnedEmptyPayload:
                Array.isArray(whisperPayload?.segments) &&
                whisperPayload.segments.length > 0
                  ? 0
              ...
              lastChunkErrorMessage: "",
              sttAttempts: Array.isArray(sttResult?.attempts)
                ? sttResult.attempts
                : [],
            };
          }
          if (DEBUG)
            console.log(`[pipeline] Transcription succeeded with model: ${model}`);
          break;
        } catch (error: any) {
          whisperPayload = null;
          ...
          transcriptionDiagnostics = {
            ...
            lastChunkErrorMessage: clean(
              error?.transcriptionDiagnostics?.lastChunkErrorMessage ||
                error?.message ||
                ""
            ),
          ...
          console.error(
            `[pipeline] Transcription failed with model ${model}:`,
            error.message
          );
      ...
      if (!whisperPayload && _sttUseGroq && OPENAI_API_KEY) {
        ...
        for (const model of openaiModels) {
          ...
          try {
            console.log(
              `[pipeline] Groq STT failed — retrying with OpenAI model ${model}`
            );
            if (isLargeFile) {
              const chunkPayloads = await transcribeInChunks(
                transcribeFilePath,
            ...
            if (whisperPayload) {
              ...
              if (DEBUG)
                console.log(
                  `[pipeline] OpenAI fallback succeeded with model: ${model}`
                );
              break;
            }
          } catch (e: any) {
            ...
            console.error(
              `[pipeline] OpenAI fallback failed for model ${model}:`,
              e.message
            );
          }
        }
      ...
      const verificationSegments = normalizeVerificationSegments(
        whisperPayload || {}
      );
      ...
      if (!diarization || !diarization.segments.length) {
        if (DEBUG)
          console.log("[pipeline] Using whisper segments as single-speaker fallback.");
        ...
        diarization.segments = diarization.segments.filter((seg: any) => {
          const hasSpeech = speechSegments.some((v: any) => {
            const overlap = Math.max(
              0,
              Math.min(seg.endTimestamp, v.end) -
                Math.max(seg.timestamp, v.start)
            );
            return (
              overlap > 0.1 ||
              overlap / (seg.endTimestamp - seg.timestamp) > 0.2
            );
          });
          return hasSpeech;
        });
      ...
      if (DEBUG) {
        const spkDist = verificationResult.verifiedSegments.reduce(
          (acc: any, s: any) => {
      ...
      if (voiceProfiles.length && diarization.speakerCount > 0) {
        ...
        for (const [speakerId, segs] of speakerSegmentMap.entries()) {
          const totalSpeakerTime = segs.reduce(
            (sum, s) => sum + (s.endTimestamp - s.timestamp),
            0
          );
      ...
      const processedSegments = await (async () => {
        ...
        if (
          DEBUG &&
          withoutHallucinations.length <
            verificationResult.verifiedSegments.length
        ) {
          console.log(
...
export async function transcribeRecording(asset: any, options: any = {}) {
  ...
  if (!audioQuality) {
    try {
      ...
      if (!options.signal?.aborted) {
        console.warn(
          "[pipeline] Audio quality analysis fallback failed:",
          error?.message || error
        );
      }
      audioQuality = null;
  ...
  const initialProfile: "standard" | "enhanced" =
    (audioQuality as any)?.enhancementRecommended ? "enhanced" : "standard";
  ...
  const preprocessPlan = new Map(
    attemptProfiles.map((profile) => {
      ...
      return [
        profile,
        { cacheKey, cachePath: getPreprocessCachePath(cacheKey, profile) },
      ];
    })
  );
  ...
  try {
    for (let index = 0; index < attemptProfiles.length; index += 1) {
      ...
      const profileWorkingFilePath =
        plan?.cachePath && fs.existsSync(plan.cachePath)
          ? plan.cachePath
          : sourceFilePath;
      const profileOptions = {
        ...
        workingContentType:
          profileWorkingFilePath === plan?.cachePath
            ? "audio/wav"
            : asset.content_type,
        preprocessedFilePath:
          profileWorkingFilePath === plan?.cachePath ? plan.cachePath : "",
      ...
      try {
        ...
        if (
          ...
          if (DEBUG) {
            console.log(
              "[pipeline] Retrying transcription with enhanced preprocessing profile."
            );
          }
          continue;
        }
    ...
    return (
      ...
      buildEmptyTranscriptResult(
        "no_segments_from_stt",
        {
          ...
          transcriptionAttemptCount: Math.min(
            attemptProfiles.length,
            2
          ) as 1 | 2,
        },
