...
Zwróć TYLKO jedno słowo: gemini, github, claude, gpt lub groq.`;
  ...
  const finalAgent = (state.task.toLowerCase().includes("repozytorium") || state.task.toLowerCase().includes("github")) 
    ? "github" 
    : (validAgents.includes(assigned) ? assigned : "gpt");
...
const handleGithub = async (state: typeof DispatcherState.State) => {
  try {
    ...
    if (res1.tool_calls && res1.tool_calls.length > 0) {
      ...
      const files = Array.isArray(data) ? data.map((f: any) => f.name).join(", ") : "To nie jest katalog";
      
      // Drugie zapytanie - dajemy modelowi wynik z GitHuba
      const finalRes = await rawModel.invoke([
        new HumanMessage(state.task),
        new SystemMessage(`System pobrał wynik z GitHuba. W repozytorium ${owner}/${repo} znajdują się następujące pliki: ${files}. Odpowiedz użytkownikowi na jego pytanie. Jeśli pytał o ilość - policz je.`)
      ]);
...
const handleGpt = async (state: typeof DispatcherState.State) => {
  ...
  const response = await model.invoke([
    new SystemMessage(state.error ? `BACKUP MODE: Poprzedni agent zawiódł (${state.error}).` : "Jesteś asystentem AI."),
    new HumanMessage(state.task)
  ]);
  return { result: response.content.toString(), messages: [response], error: "" };
};
...
const workflow = new StateGraph(DispatcherState)
  ...
  .addConditionalEdges("gemini_agent", (state) => state.error ? "fallback" : "end", { fallback: "gpt_agent", end: END })
  .addConditionalEdges("github_agent", (state) => state.error ? "fallback" : "end", { fallback: "gpt_agent", end: END })
  .addConditionalEdges("claude_agent", (state) => state.error ? "fallback" : "end", { fallback: "gpt_agent", end: END })
  .addConditionalEdges("groq_agent", (state) => state.error ? "fallback" : "end", { fallback: "gpt_agent", end: END })
