...
function buildWorkspaceDigest(state: any, workspaceName = "") {
  ...
  const overdueTasks = tasks
    .filter((task) => !isCompletedTask(task) && task.dueDate && isoDateOnly(task.dueDate) && isoDateOnly(task.dueDate) < today)
...
function renderDigestText(userName: string, digests: ReturnType<typeof buildWorkspaceDigest>[]) {
  ...
  digests.forEach((digest) => {
    ...
    digest.overdueTasks.forEach((task: DigestTask) => lines.push(`  - ${task.title}${task.dueDate ? ` (${task.dueDate})` : ""}`));
    ...
    digest.upcomingMeetings.forEach((meeting: DigestMeeting) => lines.push(`  - ${meeting.title}${meeting.startsAt ? ` (${meeting.startsAt})` : ""}`));
  });
...
export function createDigestRoutes(services: AppServices, middlewares: AppMiddlewares) {
  ...
  router.get("/daily", applyRateLimit("digest-daily", 5), async (c) => {
    ...
    const fromAddress = String(process.env.VOICELOG_SMTP_FROM || process.env.VOICELOG_SMTP_USER || "no-reply@voicelog.local").trim();
    const previews: any[] = [];
    let sent = 0;
    let skipped = 0;
