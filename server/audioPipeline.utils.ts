 ...
 */
export function mergeShortSegments<T extends { 
  text: string; 
  timestamp: number; 
  endTimestamp: number; 
    ...
    if (duration < minDuration && seg.speakerId === pending.speakerId) {
      pending = {
        ...
        verificationScore: Math.min(
          pending.verificationScore ?? 1, 
          seg.verificationScore ?? 1
        ),
        verificationStatus: 
...
export function synthesizeSegmentsFromWords(payload: any): { 
  ...
  if (words.length) {
    ...
    const flushCurrent = () => {
      ...
      const joinedText = current.words.map((word: any) => word.text).join(" ").trim();
...
export function normalizeDiarizedSegments(payload: any): {
  ...
  return {
    ...
    text: clean(payload?.text || payload?.transcript || synthesized?.text || segments.map((segment: any) => segment.text).join(" ")),
  };
}
...
export function evaluateAgainstVerificationPass(
  ...
  const overlaps = verificationSegments.filter((candidate) => 
    overlapSeconds(segment, candidate) > 0.08
  );
...
export function buildEmptyTranscriptResult(
  reason: "no_segments_from_stt" | "segments_removed_by_vad" | "segments_removed_as_hallucinations" | "all_chunks_discarded_as_too_small",
