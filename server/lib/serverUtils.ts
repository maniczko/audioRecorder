...
export function corsHeaders(requestOrigin: string, allowedOrigins = 'http://localhost:3000') {
  ...
  const matchesAllowedWildcard = allowed.some((originPattern) => {
    ...
    if (
      ...
      /vercel\.app/i.test(originPattern) &&
  ...
  const allowVercelPreview = process.env.VOICELOG_ALLOW_VERCEL_PREVIEWS === 'true' || !isProduction;
  const isVercel = allowVercelPreview && /^https:\/\/[a-z0-9.-]+\.vercel\.app$/i.test(src);
  const fallbackOrigin =
    allowed.find((originPattern) => originPattern && !originPattern.includes('*')) || '';

  const origin =
    (!isProduction && isLocalhost) ||
    isVercel ||
    (!isProduction && allowAny) ||
    allowed.includes(src) ||
    matchesAllowedWildcard
      ? src
      : fallbackOrigin;
