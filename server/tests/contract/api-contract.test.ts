...
import { parse } from 'yaml'; // Changed import from YAML to parse

// Load OpenAPI spec
const openapiPath = path.resolve(__dirname, '../../../docs/openapi.yaml');
const openapiSpec = parse(fs.readFileSync(openapiPath, 'utf-8')); // Updated to use parse
...
describe('API Contract Tests', () => {
  describe('OpenAPI Specification Validation', () => {
    ...
    it('has all required paths defined', () => {
      ...
      expect(paths).toContain('/api/'); // Fixed missing path check
    });
  });
});