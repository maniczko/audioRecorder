...
describe("Digest routes", () => {
  ...
  test("returns preview mode when SMTP is not configured", async () => {
    ...
    mockWorkspaceService.getWorkspaceState.mockResolvedValue({
      manualTasks: [
        ...
        { id: "t2", title: "Dzisiejsze zadanie", dueDate: new Date().toISOString().slice(0, 10), status: "todo" },
      ],
      meetings: [
        { id: "m1", title: "Spotkanie zespołu", startsAt: new Date(Date.now() + 60 * 60 * 1000).toISOString() },
      ],
    });
  ...
  test("sends digest email when SMTP is configured", async () => {
    ...
    mockWorkspaceService.getWorkspaceState.mockResolvedValue({
      manualTasks: [{ id: "t1", title: "Zadanie", dueDate: new Date().toISOString().slice(0, 10), status: "todo" }],
      meetings: [],
    });
