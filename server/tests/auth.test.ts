...
describe("Authentication Logic (Server Layer)", () => {
  ...
  test("should fail login with wrong password", async () => {
    ...
    await expect(db.loginUser({ email, password: "wrong_password" }))
      .rejects.toThrow("Niepoprawny email lub haslo.");
  });

  test("should fail login with non-existent email", async () => {
    await expect(db.loginUser({ email: "missing@example.com", password: "any" }))
      .rejects.toThrow("Niepoprawny email lub haslo.");
  });
  ...
  test("should reject expired session (simulated)", async () => {
    ...
    await db._execute("UPDATE sessions SET expires_at = ? WHERE token = ?", [pastDate, result.token]);
  ...
  test("should support multiple workspaces and selecting preferred one", async () => {
    ...
    const ws2Id = "ws_2";
    await db._execute("INSERT INTO workspaces (id, name, owner_user_id, invite_code, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)", [ws2Id, "WS 2", userId, "INV2", db.nowIso(), db.nowIso()]);
    await db._execute("INSERT INTO workspace_members (workspace_id, user_id, member_role, joined_at) VALUES (?, ?, 'owner', ?)", [ws2Id, userId, db.nowIso()]);
    ...
    const login2 = await db.loginUser({ email: "multi@example.com", password: "password123", workspaceId: ws2Id });
