...
describe('TranscriptionService - Additional Coverage', () => {
  ...
  describe('getSpeakerAcousticFeatures()', () => {
    ...
    test('throws error when pipeline does not support acoustic features analysis', async () => {
      ...
      await expect(service.getSpeakerAcousticFeatures(mockAsset)).rejects.toThrow(
        'Audio pipeline nie wspiera metryk akustycznych.'
      );
    });
  ...
  describe('analyzeAudioQuality()', () => {
    test('calls pipeline.analyzeAudioQuality', async () => {
      ...
      expect(mockAudioPipeline.analyzeAudioQuality).toHaveBeenCalledWith('/tmp/audio.wav', {});
    });
    ...
    test('throws error when pipeline does not support audio quality analysis', async () => {
      ...
      await expect(service.analyzeAudioQuality('/tmp/audio.wav')).rejects.toThrow(
        'Audio pipeline nie wspiera analizy jakosci audio.'
      );
    });
  });
  ...
  describe('queryRAG()', () => {
    test('queries RAG with question and returns top chunks', async () => {
      ...
      expect(mockAudioPipeline.embedTextChunks).toHaveBeenCalledWith(['What is relevant?']);
  ...
  describe('diarizeFromTranscript()', () => {
    test('calls pipeline.diarizeFromTranscript', async () => {
      ...
      expect(mockAudioPipeline.diarizeFromTranscript).toHaveBeenCalledWith(whisperSegments, {});
    });
  });

  describe('transcribeLiveChunk()', () => {
    test('calls pipeline.transcribeLiveChunk', async () => {
      mockAudioPipeline.transcribeLiveChunk.mockResolvedValue('Transcribed text');
      ...
      const result = await service.transcribeLiveChunk('/tmp/chunk.webm', 'audio/webm');
  ...
  describe('analyzeMeetingWithOpenAI()', () => {
    test('calls pipeline.analyzeMeetingWithOpenAI', async () => {
      ...
      expect(mockAudioPipeline.analyzeMeetingWithOpenAI).toHaveBeenCalledWith(data);
    });
  });

  describe('generateVoiceCoaching()', () => {
    test('calls pipeline.generateVoiceCoaching', async () => {
      mockAudioPipeline.generateVoiceCoaching.mockResolvedValue('Good diction advice');
      ...
      const result = await service.generateVoiceCoaching(asset, '1', segments);
  ...
  describe('normalizeRecording()', () => {
    test('calls pipeline.normalizeRecording', async () => {
      ...
      expect(mockAudioPipeline.normalizeRecording).toHaveBeenCalledWith('/tmp/audio.wav', {});
    });
  });
  ...
  describe('computeEmbedding()', () => {
    test('uses injected speakerEmbedder', async () => {
      ...
      expect(mockSpeakerEmbedder.computeEmbedding).toHaveBeenCalledWith('/tmp/audio.wav');
    });
  });
  ...
  describe('db wrapper methods', () => {
    ...
    test('saveAudioQualityDiagnostics delegates to db.saveAudioQualityDiagnostics', async () => {
      ...
      expect(mockDb.saveAudioQualityDiagnostics).toHaveBeenCalledWith('rec1', {
        qualityLabel: 'good',
      });
    });
  });
