...
describe('Audio Pipeline Full Integration Flow', () => {
    ...
    mockTranscriptionService = {
      ...
      ensureTranscriptionJob: vi.fn().mockImplementation(async (id, asset, body) => {
        ...
        mockTranscriptionService.getMediaAsset.mockResolvedValueOnce({
          ...
          diarization_json: '{"speakerCount": 2, "speakerNames": {"0": "A", "1": "B"}}',
  ...
  it('Full scenario: Upload -> Transcribe -> Review Status -> Fallback Failure', async () => {
    ...
    expect(eventEmitterLogs).toEqual(
      expect.arrayContaining([
        { event: 'queue', id: 'rec_flow_1' },
        { event: 'job_started', id: 'rec_flow_1' },
      ])
    );

    // 3. Review Status (The mock in `ensureTranscriptionJob` shifted the DB status to "completed")
    const resStatus = await app.request('/media/recordings/rec_flow_1/transcribe', {
      method: 'GET',
    ...
    mockTranscriptionService.getMediaAsset
      ...
      .mockResolvedValueOnce({
        ...
        diarization_json: '{"errorMessage": "AI unreachable"}',
      });
    ...
    const resStatusFailed = await app.request('/media/recordings/rec_flow_failed/transcribe', {
      method: 'GET',
      headers: { Authorization: 'Bearer fake' },
    });
