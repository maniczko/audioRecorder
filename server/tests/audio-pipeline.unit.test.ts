...
describe('audioPipeline exports', () => {
  ...
  it('caches preprocessed audio and reuses the cached file on subsequent calls', async () => {
    ...
    const firstPath = await pipeline.preprocessAudio(sourcePath, undefined, 'standard', {
      cacheKey,
    });
    const secondPath = await pipeline.preprocessAudio(sourcePath, undefined, 'standard', {
      cacheKey,
    });
                    ...
                    categoryScores: [
                      {
                        ...
                        score: 8,
                        observation: 'Dobrze prowadzone',
                        improvementTip: 'Domykaj szybciej',
                      },
                    ],
                  },
                }),
  ...
  it('extracts speaker audio clips, normalizes audio and generates voice coaching with mocked exec/fs', async () => {
    const execMock = vi
      .fn()
      .mockImplementation((_cmd, _opts, callback) => callback?.(null, '', ''));
  ...
  it('surfaces voice coaching API errors and still cleans up temp clip', async () => {
    ...
    await expect(
      pipeline.generateVoiceCoaching({ id: 'rec1', file_path: '/tmp/audio.wav' }, '1', [
        { speakerId: '1', timestamp: 0, endTimestamp: 2 },
      ])
  ...
  it('analyzeAudioQuality returns good quality metrics from ffprobe and volumedetect', async () => {
    ...
    vi.doMock('node:child_process', () => {
      ...
      const getIO = (cmdStr: string): [string, string] => {
        ...
        if (cmdStr.includes('volumedetect'))
          return ['', 'mean_volume: -18.5 dB\nmax_volume: -3.2 dB'];
  ...
  it('analyzeAudioQuality classifies poor quality when volume is very low and silence ratio is high', async () => {
    ...
    vi.doMock('node:child_process', () => {
      ...
      const getIO = (cmdStr: string): [string, string] => {
        ...
        if (cmdStr.includes('volumedetect'))
          return ['', 'mean_volume: -38.0 dB\nmax_volume: -20.0 dB'];
        if (cmdStr.includes('silencedetect'))
          return ['', 'silence_duration: 12.0\nsilence_duration: 12.0'];
        return ['', ''];
      };
      const exec = vi.fn().mockImplementation((cmd: any, _opts: any, cb: any) => {
        const [stdout, stderr] = getIO(String(cmd || ''));
  ...
  it('normalizeRecording cleans up tmp file and rethrows when ffmpeg fails', async () => {
    ...
    const pipeline = await import('../audioPipeline.ts');
    await expect(pipeline.normalizeRecording('/tmp/audio.wav')).rejects.toThrow(
      'ffmpeg: No such file'
    );
  ...
  // -----------------------------------------------------------------
  // Issue #0 - voice profile smoke can fail after audio download succeeds
  // Date: 2026-05-22
  // Bug: production could materialize a short remote recording, but a clip
  //      extraction failure still surfaced as audio_source_unavailable.
  // Fix: for short, single-speaker samples, fall back to the materialized
  //      source audio instead of failing the enrollment.
  // -----------------------------------------------------------------
  it('Regression: extractSpeakerAudioClip uses whole short source when ffmpeg clip extraction fails', async () => {
    vi.resetModules();
    const downloadAudioToFile = vi.fn().mockResolvedValue(undefined);
    ...
    const existsSync = vi.fn((filePath: string) => !String(filePath).includes('recording_remote'));
    const execMock = vi.fn().mockImplementation((_cmd, _opts, callback) => {
      callback?.(new Error('ffmpeg extraction failed'), '', 'ffmpeg extraction failed');
      return { stdout: { on: vi.fn() }, on: vi.fn() };
    });
    ...
    const clipPath = await pipeline.extractSpeakerAudioClip(asset, 'speaker_1', [
      { speakerId: 'speaker_1', timestamp: 0, endTimestamp: 8, text: 'Pierwsza probka.' },
      { speakerId: 'speaker_1', timestamp: 8, endTimestamp: 18, text: 'Druga probka.' },
    ]);

    expect(clipPath).toContain('speaker_recording_remote_speaker_1_');
    expect(execMock).toHaveBeenCalled();
    expect(copyFileSync).toHaveBeenCalledWith(
      expect.stringContaining('temp_voice_profile_source_'),
      expect.stringContaining('speaker_recording_remote_speaker_1_')
    );
  }, 10000);
  ...
  it('extractSpeakerAudioClip throws error when no valid segments', async () => {
    ...
    await expect(pipeline.extractSpeakerAudioClip(asset, '1', segments)).rejects.toThrow(
      'Brak segmentów z poprawnymi znacznikami czasu'
    );
  });
  ...
  it('analyzeAcousticFeatures throws error when file does not exist', async () => {
    ...
    await expect(pipeline.analyzeAcousticFeatures('/nonexistent.wav')).rejects.toThrow(
  ...
  it('transcribeRecording uses pyannote for diarization and exercises word-level speaker splitting', async () => {
    ...
    const asset = {
      ...
      diarization_json: JSON.stringify({
        audioQuality: { qualityLabel: 'good', enhancementRecommended: false, qualityScore: 80 },
      }),
    };
    ...
    (global.fetch as any)
      ...
      .mockResolvedValueOnce({
        ...
        text: vi.fn().mockResolvedValue(
          JSON.stringify({
            text: 'To jest test',
            segments: [{ text: 'To jest test', start: 0, end: 2 }],
          })
        ),
      })
