  ...
  describe('uploadAudioToStorage()', () => {
    ...
    test('sanitizes recording ID with special characters', async () => {
      ...
      expect(mockUpload).toHaveBeenCalledWith(
        'rec_test_123.webm', expect.any(Buffer), {
          contentType: 'audio/webm',
          upsert: true,
        }
      );
    ...
    // Fixing the test that expects a rejected promise
    test('rejects promise on upload error', async () => {
      mockCreateBucket.mockResolvedValueOnce({ data: {}, error: null });
      mockUpload.mockResolvedValueOnce({
        data: null,
      ...
      await expect(uploadAudioToStorage('rec1', Buffer.from('test-audio'), 'audio/webm', '.webm'))
