...
describe("TranscriptionService Reproduction", () => {
  it("should throw a clear error if pipeline is null", async () => {
    ...
    const service = new TranscriptionService(mockDb, mockWorkspaceService, null, mockSpeakerEmbedder);
    ...
    await expect(act()).rejects.toThrow("Critical: TranscriptionService.audioPipeline is null or undefined.");
  });
  ...
  it("should throw a clear error if pipeline is missing transcribeRecording", async () => {
    ...
    const service = new TranscriptionService(mockDb, mockWorkspaceService, brokenPipeline, mockSpeakerEmbedder);
    ...
    await expect(act()).rejects.toThrow("Critical: TranscriptionService.audioPipeline is missing 'transcribeRecording'.");
  });
});
