  ...
  test("import Google Tasks z mock API importuje zadania", async ({ page }) => {
    ...
    await page.route("**/oauth2.googleapis.com/**", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({ access_token: "mock_token", token_type: "Bearer", expires_in: 3600 }),
      })
    );
    ...
    await page.route("**/tasks.googleapis.com/tasks/v1/lists/*/tasks**", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          items: [
            { id: "gtask_1", title: "Zadanie z Google", status: "needsAction", updated: new Date().toISOString() },
          ],
        }),
      })
    );
