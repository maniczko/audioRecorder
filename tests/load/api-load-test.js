...
function executeAIFlow(authToken, workspaceId) {
  ...
  const searchRes = http.post(`${BASE_URL}/ai/search`, JSON.stringify({
  ...
  const summary = {
    ...
    metrics: {
      ...
      success_rate: data.metrics.successful_requests ? 
        (data.metrics.successful_requests.values.count / data.metrics.total_requests.values.count * 100).toFixed(2) : 0,
