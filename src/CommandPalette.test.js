...
describe("CommandPalette", () => {
  ...
  test("calls onSelect when an item is clicked", async () => {
    ...
    expect(props.onSelect).toHaveBeenCalledWith(props.items[0]);
  });
  ...
  test("supports keyboard arrow navigation", async () => {
    ...
    // Indeks 1 to 'Zadanie Test' po naciśnięciu w dół
    expect(props.onSelect).toHaveBeenCalledWith(props.items[1]);
  });
});
