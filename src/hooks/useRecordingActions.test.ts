...
describe("useRecordingActions", () => {
  ...
  const baseMeeting = {
    ...
    recordings: [
      { id: "r1", transcript: [{ id: "s1", speakerId: "0", text: "Test", timestamp: 0, verificationStatus: "review" }], speakerNames: { "0": "Speaker 1" }, markers: [] },
    ],
  ...
  test("addMeetingComment and attachCompletedRecording", () => {
    ...
    act(() => {
      result.current.attachCompletedRecording("m1", { id: "r2", analysis: { suggestedTags: ["AI"] } });
    });
    newMeetings = mockSetMeetings.mock.calls[1][0](newMeetings);
  ...
  test("mergeTranscriptSegments and splitTranscriptSegment", () => {
    const customMeeting = {
      ...
      recordings: [
        ...
        ], speakerNames: { "0": "Speaker 1" }, markers: [] },
      ],
    };
