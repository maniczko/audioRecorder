...
export default function useMeetingLifecycle({
  ...
  useEffect(() => {
    if (!currentUserId || !currentWorkspaceId) {
      ...
      skipSelectedMeetingDraftSyncRef.current = ''; // Fixed missing assignment
    }
  }, [currentUserId, currentWorkspaceId, selectedMeeting, selectedRecording]); // Added missing dependencies
}
```