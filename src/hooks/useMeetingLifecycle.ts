...
export default function useMeetingLifecycle({
  ...
  useEffect(() => {
    ...
    const freshDraft = activeStoredMeetingDraft || createEmptyMeetingDraft();
    if (!areMeetingDraftsEqual(freshDraft, draftBaselineRef.current)) {
      setMeetingDraft(freshDraft);
      draftBaselineRef.current = freshDraft;
    }
  }, [currentUserId, currentWorkspaceId, activeStoredMeetingDraft, setMeetingDraft, selectedMeeting, selectedRecording]); // Updated dependencies
  ...
  return {
    ...
    setIsDetachedMeetingDraft,
    hasMeetingDraftChanges,
    setHasMeetingDraftChanges,
  };
}
```