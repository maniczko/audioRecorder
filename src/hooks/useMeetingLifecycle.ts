...
export default function useMeetingLifecycle({
  ...
  useEffect(() => {
    if (!currentUserId || !currentWorkspaceId) {
      ...
      return; // Ensure to return early if conditions are met
    }

    // Additional logic for restoring drafts can go here

  }, [currentUserId, currentWorkspaceId, selectedMeeting, selectedRecording]); // Added dependencies
