...
function uniqueTexts(values: unknown[], fallback: string[] = [], limit = 3): string[] {
  ...
  const items = [...safeArray(values), ...fallback].map((item) => cleanText(item)).filter(Boolean);
...
function getTranscriptSignals(context: MeetingFeedbackContext) {
  ...
  const avgFillerRate = speakerStats.length
    ? avg(
        speakerStats
          .map((stat) => Number(stat.fillerRate ?? 0))
          .filter((value) => Number.isFinite(value))
      )
    : 0;
  const avgWpm = speakerStats.length
    ? avg(
        speakerStats.map((stat) => Number(stat.wpm ?? 0)).filter((value) => Number.isFinite(value))
      )
    : 0;
...
function makeCategoryScores(context: MeetingFeedbackContext): MeetingFeedbackCategoryScore[] {
  ...
  const balanceBonus =
    dominantTalkRatio > 0 && dominantTalkRatio < 0.72 ? 1 : dominantTalkRatio >= 0.86 ? -1 : 0;
  ...
  const categoryScores: Record<FeedbackCategoryKey, number> = {
    ...
    pace:
      ...
      (speakerStats.length > 0
        ? signals.speakerStats.some((stat) => Number(stat.avgTurnSeconds || 0) > 90)
          ? -0.4
          : 0.3
        : 0),
    collaboration:
...
function buildCategoryObservation(
  categoryKey: FeedbackCategoryKey,
  score: number,
  signals: ReturnType<typeof getTranscriptSignals>
): string {
  const {
    decisions,
    actionCount,
  ...
  const lowDataNote = hasEnoughData
    ? ''
    : ' Dane są ograniczone, więc ocena ma charakter orientacyjny.';
...
function buildStrengths(
  context: MeetingFeedbackContext,
  signals: ReturnType<typeof getTranscriptSignals>
): string[] {
  ...
  if (signals.actionCount > 1) {
    strengths.push(
      'Pojawiło się kilka użytecznych ustaleń, które można od razu zamienić w działania.'
    );
  }
  ...
  if (signals.avgFillerRate <= 8 && signals.avgWpm > 0) {
    strengths.push(
      'Przekaz brzmiał względnie klarownie i bez nadmiernego przeciągania wypowiedzi.'
    );
  }
...
function buildImprovementAreas(
  ...
  if (signals.blockers.length > 0 || signals.tensions.length > 0) {
    improvementAreas.push(
      'Trzeba szybciej nazywać blokady i domykać je właścicielem oraz terminem.'
    );
  }
  if (signals.avgFillerRate > 10 || signals.avgWpm > 190) {
    improvementAreas.push(
      'Tempo i sposób wypowiadania się można jeszcze uprościć, żeby przekaz był bardziej czytelny.'
    );
  }
  if (
    (signals.participantInsights.length <= 1 || signals.dominantTalkRatio >= 0.85) &&
    signals.participantInsights.length > 0
  ) {
  ...
  if (!signals.decisions.length && cleanText(context.summary)) {
    improvementAreas.push(
      'Na końcu rozmowy powinno padać krótkie podsumowanie: co ustalono, co dalej i kto to prowadzi.'
    );
  }
...
function buildPerceptionNotes(signals: ReturnType<typeof getTranscriptSignals>): string[] {
  ...
  if (signals.avgFillerRate > 10 || signals.avgWpm > 190) {
    notes.push(
      'Możesz być odbierany jako ktoś, kto mówi szybko i czasem zostawia za mało przestrzeni na odpowiedź.'
    );
  } else {
  ...
  if (signals.actionCount > 0 || signals.decisions.length > 0) {
    notes.push(
      'Dajesz wrażenie osoby nastawionej na domykanie spraw i szukanie praktycznego efektu.'
    );
  } else {
  ...
  if (signals.tensions.length > 0) {
    ...
    notes.push(
      'Możesz być odbierany jako partner do rozmowy, jeśli częściej podsumowujesz cudze wypowiedzi.'
    );
  }
...
function buildCommunicationTips(signals: ReturnType<typeof getTranscriptSignals>): string[] {
  ...
  if (signals.avgFillerRate > 10 || signals.avgWpm > 190) {
    ...
    tips.push(
      'Przed przejściem do następnego tematu zamknij poprzedni jednym zdaniem podsumowania.'
    );
  }

  if (signals.participantInsights.length > 1) {
    tips.push(
      'Zostaw więcej miejsca na reakcję: pytanie, cisza, odpowiedź, dopiero potem kolejny krok.'
    );
  } else {
    tips.push(
      'Zamiast rozwijać wątek od razu, dopytaj o jeden konkretny punkt i podsumuj go na głos.'
    );
  }
...
function buildNextSteps(
  ...
  if (signals.avgFillerRate > 10 || signals.avgWpm > 190) {
    ...
    nextSteps.push(
      'Na następnym spotkaniu użyj krótkiego podsumowania po każdym bloku tematycznym.'
    );
  }
  if (!signals.decisions.length) {
    ...
    nextSteps.push(
      `Przygotuj krótką agendę do spotkania "${context.meetingTitle}" z jednym jasnym celem.`
    );
  }
...
function buildWhatWentWell(
  ...
  if (signals.participantInsights.length > 1)
    items.push('W spotkaniu było miejsce na kilka perspektyw.');
  if (!signals.tensions.length) items.push('Atmosfera była raczej spokojna i współpracująca.');
  if (!items.length && cleanText(context.summary))
    items.push('Dało się uchwycić główny temat rozmowy.');
  return uniqueTexts(items, [], 3);
}

function buildWhatCouldBeBetter(
  context: MeetingFeedbackContext,
  signals: ReturnType<typeof getTranscriptSignals>
): string[] {
  const items: string[] = [];
  if (signals.actionCount === 0)
    items.push('Brakuje mocniejszego przejścia od rozmowy do konkretnego działania.');
  if (signals.avgFillerRate > 10 || signals.avgWpm > 190)
    items.push('Wypowiedzi można skrócić i uprościć, żeby były bardziej czytelne.');
  if (signals.dominantTalkRatio >= 0.85) items.push('Warto częściej oddawać głos drugiej stronie.');
  if (signals.tensions.length > 0 || signals.blockers.length > 0)
    items.push('Trzeba szybciej domykać sporne lub blokujące wątki.');
  if (!signals.decisions.length)
    items.push('Na końcu rozmowy powinno paść krótkie podsumowanie decyzji i właścicieli.');
  if (!items.length && cleanText(context.summary))
    items.push('Można jeszcze bardziej wyostrzyć cel spotkania i kończyć każdy temat decyzją.');
  return uniqueTexts(items, [], 3);
}
...
export function normalizeMeetingFeedback(
  rawFeedback: unknown,
  context: MeetingFeedbackContext
): MeetingFeedback {
  ...
  const raw = rawFeedback as Partial<MeetingFeedback> & {
