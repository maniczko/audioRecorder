...
const EMPTY_AUTH_DRAFT = { name: "", role: "", company: "", email: "", password: "", workspaceMode: "create", workspaceName: "", workspaceCode: "" };
...
export const useAuthStore = create<any>((set, get) => ({
  ...
  requestResetCode: async () => {
    ...
    try {
      ...
      if (result.users) setUsers(result.users);
      set({ resetPreviewCode: result.recoveryCode || "", resetExpiresAt: result.expiresAt || "", resetMessage: "Kod resetu jest gotowy. Ustaw nowe haslo ponizej." });
    } catch (error: any) {
      set({ authError: error.message });
    }
  },
  ...
  completeReset: async () => {
    ...
    try {
      ...
      set({ 
        ...
        resetPreviewCode: "", resetExpiresAt: "", 
  ...
  saveProfile: async (currentUser) => {
    ...
    const { users, setUsers } = useWorkspaceStore.getState();
    try {
      const result = await authService.updateProfile({ users, userId: currentUser.id, updates: profileDraft });
  ...
  updatePassword: async (currentUser) => {
    ...
    const { users, setUsers } = useWorkspaceStore.getState();
    try {
      const result = await authService.changePassword({ users, userId: currentUser.id, draft: passwordDraft });
      if (Array.isArray(result)) setUsers(result);
      else if (Array.isArray(result?.users)) setUsers(result.users);
      else if (result?.user) setUsers((prev) => mergeUserIntoCollection(prev, result.user));
