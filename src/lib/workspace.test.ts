...
describe('workspace helpers', () => {
  ...
  it('collects members and workspace ids', () => {
    ...
    expect(userWorkspaceIds({ id: 'user_2', workspaceIds: ['extra'] }, [joined])).toEqual([
      'extra',
      joined.id,
    ]);
  });
  ...
  it('resolves preferred and default workspace ids', () => {
    const workspaces = [createWorkspace('One', 'user_1'), createWorkspace('Two', 'user_1')];
    const user = {
      id: 'user_1',
  ...
  it('migrates workspace data for users and sessions', () => {
    ...
    expect(result.taskBoards[existingWorkspace.id]).toEqual({
