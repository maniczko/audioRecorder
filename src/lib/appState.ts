...
export function normalizeTaskUpdatePayload(previousTask, updates, columns) {
  ...
  let assignedTo =
    updates.assignedTo === undefined
      ? normalizeTaskPeopleList(previousTask.assignedTo?.length ? previousTask.assignedTo : previousTask.owner)
      : normalizeTaskPeopleList(updates.assignedTo);

  if (updates.owner !== undefined) {
    const nextOwner = String(updates.owner || "").trim();
    if (nextOwner) {
      assignedTo = [nextOwner, ...assignedTo.filter((person) => person.toLowerCase() !== nextOwner.toLowerCase())];
  ...
  return {
    ...
    completed,
    comments: updates.comments === undefined ? previousTask.comments : normalizeTaskComments(updates.comments),
    history: updates.history === undefined ? previousTask.history : normalizeTaskHistory(updates.history),
    dependencies:
