...
function buildCacheKey(query: string, items: AiSearchItem[]) {
  ...
  return JSON.stringify({
    ...
    items: normalizedItems.map((item) => [item.id, item.title, item.subtitle, item.type, item.group]),
  });
}

export async function semanticSearch(query: string, items: AiSearchItem[] = []): Promise<AiSearchResponse> {
