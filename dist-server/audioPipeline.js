...
function normalizeDiarizedSegments(payload) {
    ...
    const segments = rawSegments
        ...
        const rawSpeakerLabel = clean((segment.speaker !== undefined && segment.speaker !== null ? String(segment.speaker) : null)
...
function buildVerificationResult(diarizedSegments, verificationSegments) {
    const verifiedSegments = diarizedSegments.map((segment, index) => {
        ...
        const verificationScore = clamp(qualityScore * 0.22 + verification.whisperConfidence * 0.38 + verification.alignmentScore * 0.4, 0, 1);
        return {
            ...segment,
...
async function splitAudioIntoChunks(filePath, signal) {
    ...
    await execPromise(`"${FFMPEG_BINARY}" -y -i "${filePath}" -f segment -segment_time ${CHUNK_DURATION_SECONDS} -reset_timestamps 1 -ar 16000 -ac 1 "${chunkPattern}"`, { timeout: 300000, signal });
...
function mergeChunkedDiarizedPayloads(payloads) {
    ...
    const allSegments = payloads.flatMap(({ payload, offsetSeconds }) => {
        ...
        return segs
            .map((s) => {
            const rawSpeaker = clean((s.speaker !== undefined && s.speaker !== null ? String(s.speaker) : null)
...
async function transcribeInChunks(filePath, contentType, fields, options = {}) {
    ...
    if (DEBUG)
        console.log(`[audioPipeline] Split into ${chunks.length} chunks.`);
    const payloads = [];
    try {
        for (const chunk of chunks) {
            ...
            if (chunkSpeech && chunkSpeech.length === 0) {
                if (DEBUG)
                    console.log(`[audioPipeline] Skipping silent chunk: ${chunk.filePath} (offset: ${chunk.offsetSeconds}s)`);
...
async function preprocessAudio(filePath, signal) {
    ...
    try {
        await execPromise(`"${FFMPEG_BINARY}" -y -i "${filePath}" -af "afftdn=nf=-20:nr=0.85,highpass=f=80,lowpass=f=16000,dynaudnorm=p=0.9:m=100:s=5,aresample=resampler=swr" -ar 16000 -ac 1 "${tmpPath}"`, { timeout: 180000, signal });
        return tmpPath;
    }
    catch (err) {
        if (!signal?.aborted)
...
async function diarizeFromTranscript(segments) {
    if (!OPENAI_API_KEY || !segments.length)
        return null;
    ...
    const lines = chunk
        ...
        .join("\n");
    const systemPrompt = "You are a speaker diarization engine. Your ONLY job is to assign a speaker label (A, B, C…) " +
    ...
    try {
        ...
        if (!resp.ok)
            throw new Error(`OpenAI chat completions HTTP ${resp.status}`);
        ...
        if (!assignments.length) {
            if (DEBUG)
                console.warn("[audioPipeline] Transcript diarization: GPT returned empty assignments.");
            return null;
        }
        // Build index→speaker map; segments beyond CHUNK_SIZE inherit last known speaker
        const indexToSpeaker = new Map(assignments.map((a) => [Number(a.i), String(a.s || "A").toUpperCase().slice(0, 1)]));
        ...
        if (DEBUG)
            console.warn("[audioPipeline] diarizeFromTranscript failed:", err.message);
        return null;
    }
}
    ...
    try {
        ...
        for (const model of modelsToTry) {
            ...
            try {
                ...
                if (DEBUG)
                    console.log(`[audioPipeline] Transcription succeeded with model: ${model}`);
        ...
        if (!diarization) {
            if (DEBUG)
                console.log("[audioPipeline] Pyannote unavailable — using GPT-4o-mini transcript diarization.");
            try {
        ...
        if (!diarization || !diarization.segments.length) {
            if (DEBUG)
                console.log("[audioPipeline] Using whisper segments as single-speaker fallback.");
        ...
        if (voiceProfiles.length && diarization.speakerCount > 0) {
            ...
            for (const seg of diarization.segments) {
                ...
                if (!speakerSegmentMap.has(sid))
                    speakerSegmentMap.set(sid, []);
            ...
            for (const [speakerId, segments] of speakerSegmentMap.entries()) {
                ...
                if (totalSpeakerTime < 2)
                    continue; // not enough audio to identify
                ...
                try {
                    ...
                    const util = require("node:util");
                    const execPromise = util.promisify(exec);
                    await execPromise(`"${FFMPEG_BINARY}" -y -i "${asset.file_path}" -af "aselect='${selectFilter}',asetpts=N/SR/TB" -ar 16000 -ac 1 "${clipPath}"`, { timeout: 30000, signal: options.signal });
                    ...
                    try {
                        require("node:fs").unlinkSync(clipPath);
                    }
                    catch (_) { }
                }
            }
...
async function correctTranscriptWithLLM(segments, options = {}) {
    if (!TRANSCRIPT_CORRECTION && !options.transcriptCorrection)
        return segments;
    if (!OPENAI_API_KEY)
        return segments;
...
async function transcribeLiveChunk(filePath, contentType, options = {}) {
    ...
    try {
        ...
        if (!options.signal?.aborted) {
            if (DEBUG)
                console.warn("[audioPipeline] Live chunk transcription failed:", err.message);
        }
        return "";
    }
}
...
async function generateVoiceCoaching(asset, speakerId, segments, options = {}) {
    if (!OPENAI_API_KEY)
        throw new Error("Brak klucza OpenAI API.");
    const spkSegs = segments.filter((s) => String(s.speakerId ?? s.speaker ?? "") === String(speakerId));
    if (!spkSegs.length)
        throw new Error("Brak segmentów dla tego mówcy.");
    ...
    const validSegs = spkSegs
        ...
        .slice(0, 15);
    if (!validSegs.length)
        throw new Error("Brak segmentów z poprawnymi znacznikami czasu.");
    const clipPath = path.join(path.dirname(asset.file_path), `coaching_${asset.id}_${String(speakerId).replace(/[^a-zA-Z0-9_-]/g, "")}_clip.wav`);
    try {
        const { exec } = require("node:child_process");
        const util = require("node:util");
        const execPromise = util.promisify(exec);
        ...
        const selectFilter = validSegs
            .map((s) => `between(t,${Number(s.timestamp ?? s.start).toFixed(3)},${Number(s.endTimestamp ?? s.end).toFixed(3)})`)
            .join("+");
        await execPromise(`"${FFMPEG_BINARY}" -y -i "${asset.file_path}" -af "aselect='${selectFilter}',asetpts=N/SR/TB" -t 60 -ar 16000 -ac 1 "${clipPath}"`, { timeout: 30000, signal: options.signal });
        ...
        if (!coaching)
            throw new Error("Pusta odpowiedź z modelu audio.");
        return coaching;
    }
    finally {
        try {
...
async function normalizeRecording(filePath, options = {}) {
    ...
    try {
        await execPromise(`"${FFMPEG_BINARY}" -y -i "${filePath}" -af loudnorm=I=-16:TP=-1.5:LRA=11 "${tmpPath}"`, { timeout: 120000, signal: options.signal });
...
async function analyzeMeetingWithOpenAI({ meeting, segments, speakerNames }) {
    ...
    try {
        ...
        if (!resp.ok)
            throw new Error(`OpenAI analyze HTTP ${resp.status}`);
        const json = await resp.json();
        const content = json.choices?.[0]?.message?.content || "{}";
