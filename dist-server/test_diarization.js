...
const syntheticSegments = [
    ...
    { text: "Najbardziej lubię rozwiązywać skomplikowane problemy algorytmiczne.", start: 13, end: 18 },
    ...
    { text: "Głównie JavaScript i TypeScript, ale też trochę Python do skryptów.", start: 34, end: 40 },
...
async function diarizeFromTranscript(segments) {
    ...
    const lines = chunk
        ...
        .join("\n");
    const systemPrompt = "Jesteś ekspertem od analizy nagrań spotkań. Identyfikujesz mówców na podstawie struktury rozmowy.";
    const userPrompt = [
    ...
    console.log("");
    if (!resp.ok)
        throw new Error(`OpenAI HTTP ${resp.status}: ${rawText.slice(0, 200)}`);
...
async function main() {
    console.log(`Testowanie diaryzacji na ${syntheticSegments.length} segmentach syntetycznej rozmowy PL...`);
    console.log("");
    try {
        ...
        if (speakers.length < 2) {
            console.warn(`PROBLEM: Tylko ${speakers.length} mówca(y) wykryto — oczekiwano 2 dla tej rozmowy Q&A.`);
        }
        else {
        ...
        const evenDominant = evenSpeakers.sort((a, b) => evenSpeakers.filter((s) => s === b).length - evenSpeakers.filter((s) => s === a).length)[0];
        const oddDominant = oddSpeakers.sort((a, b) => oddSpeakers.filter((s) => s === b).length - oddSpeakers.filter((s) => s === a).length)[0];
