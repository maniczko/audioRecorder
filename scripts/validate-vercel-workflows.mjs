...
const hasProductionTrigger =
  /workflow_run:/.test(productionWorkflow) ||
  /push:\s*\n\s*branches:\s*\[main\]/m.test(productionWorkflow);
if (!hasProductionTrigger) {
  throw new Error(
    `Workflow validation failed for ${productionWorkflowPath}: missing push or workflow_run trigger on main`
  );
}
assertMatch(
  productionWorkflow,
...
for (const [relativePath, content] of [
  ...
  assertMatch(
    content,
    relativePath,
    'Validate Vercel secrets step',
    /name:\s*Validate Vercel secrets/m
  );
  assertMatch(
    content,
    relativePath,
    'VERCEL_TOKEN env reference',
    /VERCEL_TOKEN:\s*\$\{\{\s*secrets\.VERCEL_TOKEN\s*\}\}/m
  );
  assertMatch(
    content,
    relativePath,
    'VERCEL_ORG_ID env reference',
    /VERCEL_ORG_ID:\s*\$\{\{\s*secrets\.VERCEL_ORG_ID\s*\}\}/m
  );
  assertMatch(
    content,
    relativePath,
    'VERCEL_PROJECT_ID env reference',
    /VERCEL_PROJECT_ID:\s*\$\{\{\s*secrets\.VERCEL_PROJECT_ID\s*\}\}/m
  );
}
...
assertMatch(
  vercelConfig,
  vercelConfigPath,
  'no-store header for index.html',
  /"source"\s*:\s*"\/index\.html"[\s\S]*?"Cache-Control"[\s\S]*?"no-store,\s*max-age=0"/m
);
assertMatch(
  vercelConfig,
  vercelConfigPath,
  'no-store header for service-worker.js',
  /"source"\s*:\s*"\/service-worker\.js"[\s\S]*?"Cache-Control"[\s\S]*?"no-store,\s*max-age=0"/m
);
assertMatch(
  vercelConfig,
  vercelConfigPath,
  'no-store header for manifest.json',
  /"source"\s*:\s*"\/manifest\.json"[\s\S]*?"Cache-Control"[\s\S]*?"no-store,\s*max-age=0"/m
);
