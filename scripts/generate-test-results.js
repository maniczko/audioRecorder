...
async function main() {
  ...
  const jsonPath = path.join(rootDir, 'scripts', 'test-results.json');
  ...
  // Generate external-services.js if monitor script exists
  const monitorPath = path.join(rootDir, 'scripts', 'monitor-external-services.js');
  const externalJsPath = path.join(rootDir, 'scripts', 'external-services.js');
  if (fs.existsSync(monitorPath)) {
    console.log('\n🔗 Generating external services status...');
    try {
      const { execSync } = await import('child_process');
      execSync(`node "${monitorPath}"`, { cwd: rootDir, stdio: 'inherit' });
      if (fs.existsSync(externalJsPath)) {
        console.log('✓ External services data generated');
      } else {
        console.log('⚠️  external-services.js not created (some tokens may be missing)');
        // Create a stub so dashboard doesn't crash
        const stubContent = `// External services stub - no live data available
window.EXTERNAL_SERVICES_DATA = {
  github: { status: 'offline', actions: { recent_runs: [], latest_by_workflow: [], success_rate: 0, total_runs: 0, last_run: null }, issues: { open: [] }, pull_requests: { open: [] }, repo: { full_name: 'maniczko/audioRecorder' } },
  sentry: { status: 'offline', configured: false, issues: [], stats: null, note: 'Not configured' },
  railway: { status: 'offline', has_token: false, configured: false, note: 'No RAILWAY_TOKEN' },
  vercel: { status: 'offline', project_name: null, framework: 'Vite', note: 'No VERCEL_TOKEN' }
};
`;
        fs.writeFileSync(externalJsPath, stubContent);
      }
    } catch (error) {
      console.log('⚠️  External services monitor failed, creating stub');
      const stubContent = `// External services stub - monitor failed
window.EXTERNAL_SERVICES_DATA = {
  github: { status: 'offline', actions: { recent_runs: [], latest_by_workflow: [], success_rate: 0, total_runs: 0, last_run: null }, issues: { open: [] }, pull_requests: { open: [] }, repo: { full_name: 'maniczko/audioRecorder' } },
  sentry: { status: 'offline', configured: false, issues: [], stats: null, note: 'Monitor error' },
  railway: { status: 'offline', has_token: false, configured: false, note: 'Monitor error' },
  vercel: { status: 'offline', project_name: null, framework: 'Vite', note: 'Monitor error' }
};
`;
      fs.writeFileSync(externalJsPath, stubContent);
    }
  } else {
    // Create stub if monitor doesn't exist
    const stubContent = `// External services stub - monitor not available
window.EXTERNAL_SERVICES_DATA = {
  github: { status: 'offline', actions: { recent_runs: [], latest_by_workflow: [], success_rate: 0, total_runs: 0, last_run: null }, issues: { open: [] }, pull_requests: { open: [] }, repo: { full_name: 'maniczko/audioRecorder' } },
  sentry: { status: 'offline', configured: false, issues: [], stats: null, note: 'Not available' },
  railway: { status: 'offline', has_token: false, configured: false, note: 'Not available' },
  vercel: { status: 'offline', project_name: null, framework: 'Vite', note: 'Not available' }
};
`;
    fs.writeFileSync(externalJsPath, stubContent);
  }
  ...
  console.log(`   - scripts/external-services.js (external services status)`);
  console.log(`📊 Summary:`);
